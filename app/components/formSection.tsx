...
export default function FormSection() {
  ...
  const handleSubmit = async (e: React.SubmitEvent) => {
    ...
    if (fields.name && fields.email && fields.phone && fields.message) {
      if (
        ...
        window.open(
          `https://wa.me/2348148244729?text=${encodeURIComponent(`Good day! \nThis message was sent from a user on your site with details as follows\n\nName: ${fields.name}\nEmail: ${fields.email}\n Phone Number: ${fields.phone}\n Message: ${fields.message}`)}`,
          "_blank",
        );
      }
    }
  ...
  return (
    <>
      <section
        id="formSection"
        className="flex flex-col md:flex-row gap-y-5 md:gap-y-0 gap-x-7 items-center justify-center py-20 bg-slate-200"
      >
        <motion.div
          initial={{ x: -50, opacity: 0 }}
          whileInView={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.5 }}
          className="w-[80%] md:w-[32%] bg-white shadow-md rounded-sm p-4"
        >
        ...
        <motion.div
          ...
          whileInView={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.5 }}
          className="w-[85%] md:w-[48%] lg:w-[35%] bg-white shadow-md rounded-sm p-6"
        >
